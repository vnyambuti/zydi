...
export class MakePaymentPage implements OnInit {
  ...
  constructor(private api: RestService, private util: UtilService,private nav:NavController) {}
  ...
  // console.log(this.cartData);
    ...
    let data = {
      "orders":this.cartData
    };
    this.api.postWithHeader("orders", data).subscribe(
      (success: any) => {
        console.log(success.orders);

        if (success.orders) {
          this.nav.navigateRoot("/tabs/orders");
          localStorage.removeItem('cart-data');
          localStorage.removeItem('totalQty');
          localStorage.removeItem('totalAmount');
          this.util.presentAlert('Success','Order Made');
          this.util.stopload();
        }else{
